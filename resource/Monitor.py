...
class Monitor(Component):
    def __init__(self, size=(500, 500), position=(10, 10)):
        ...
        self.monitor_offset = (self.block_position[0] + self.monitor_bias[0],
                               self.block_position[1] + self.monitor_bias[1])  # 展示图片在屏幕坐标的偏置，方便获取鼠标位置
        self.monitor_size = (
        self.block_size[0] - self.monitor_bias[0] * 2, self.block_size[1] - self.monitor_bias[1] * 2)
    ...
    def __update_show_image_obj(self):
        ...
        if max_scale > 1.0:
            ...
            image_size_re = (
            int(floor(float(image_size[0]) * self.image_scale)), int(floor(float(image_size[1]) * self.image_scale)))
